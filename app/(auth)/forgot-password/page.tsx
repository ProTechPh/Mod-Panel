        ...
        <div className="p-6 pt-2">
          {resetData ? (
            <div className="space-y-4">
              <div className="rounded-lg p-4 space-y-2" style={{ background: 'rgba(234, 88, 12, 0.05)', border: '1px solid rgba(234, 88, 12, 0.15)' }}>
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 text-xs rounded px-2 py-1.5 truncate font-mono" style={{ background: 'rgba(2, 6, 8, 0.6)', border: '1px solid rgba(234, 88, 12, 0.15)', color: 'var(--text-mid)' }}>
                    {typeof window !== 'undefined' ? `${window.location.origin}/reset-password?token=${resetData.token}` : ''}
                  </code>
                  <button onClick={copyResetUrl} className="h-8 w-8 flex items-center justify-center rounded-md transition-all" style={{ border: '1px solid rgba(234, 88, 12, 0.2)' }}>
                    {copied ? <Check className="h-4 w-4" style={{ color: 'var(--ecto-green)' }} /> : <Copy className="h-4 w-4" style={{ color: 'var(--text-mid)' }} />}
                  </button>
                </div>
              </div>
              <button onClick={() => setResetData(null)} className="w-full h-11 rounded-lg text-sm font-medium transition-all" style={{ border: '1px solid rgba(234, 88, 12, 0.2)', background: 'rgba(2, 6, 8, 0.4)', color: 'var(--text-mid)' }}>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              <div className="space-y-2">
                ...
                <Input id="identifier" {...register('identifier')} placeholder="Enter username or email" autoComplete="username" className="placeholder:text-muted-foreground/40 transition-all" style={{ background: 'rgba(2, 6, 8, 0.6)', borderColor: 'rgba(234, 88, 12, 0.2)' }} />
