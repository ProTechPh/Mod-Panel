...
/**
 * Returns the list of games where the user (by IP) can still generate a free key.
 * Games where the user already has a non-expired active free key are excluded.
 */
export async function getAvailableGames(ip: string, registrator: string) {
  await dbConnect();

  // All enabled games for this registrator
  const allGames = await GameSetting.find({
    registrator,
    isEnabled: true,
    freeKeyEnabled: true,
  ...
  // Collect all keyIds for this IP
  const trackers = await IpTracker.find({ ipAddress: ip, isBanned: false }).lean();
  if (!trackers.length) {
    // No history — all games are available
    return allGames.map(g => ({ code: g.gameCode, name: g.gameName }));
  }

  const keyIds = trackers.map(t => t.keyId);
  const now = new Date();

  // Find which game codes this IP has an active key for
  const activeKeys = await Key.find({
    ...
    status: 1,
    expiredDate: { $gt: now },
  }).select('game -_id').lean();

  const occupiedGames = new Set(activeKeys.map(k => k.game.toUpperCase()));

  // Return only games with no active key
  return allGames
    .filter(g => !occupiedGames.has(g.gameCode.toUpperCase()))
    .map(g => ({ code: g.gameCode, name: g.gameName }));
}