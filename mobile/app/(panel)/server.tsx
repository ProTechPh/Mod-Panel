...
export default function ServerScreen() {
  ...
  return (
    <ScrollView className="flex-1 bg-background" contentContainerStyle={{ padding: 16 }}>
      ...
      <View className="bg-card border border-border/50 rounded-xl p-4 mt-4 gap-4">
        <Text className="text-lg font-semibold text-foreground">Server Info</Text>
        <View>
          <Text className="text-sm text-muted-foreground mb-1.5">Mod Name</Text>
          <TextInput
            className="bg-background border border-border rounded-lg px-4 py-3 text-foreground text-sm"
            value={config.modName}
            onChangeText={(v) => setConfig({ ...config, modName: v })}
          />
        </View>
        <View>
          <Text className="text-sm text-muted-foreground mb-1.5">Telegram Channel</Text>
          <TextInput
            className="bg-background border border-border rounded-lg px-4 py-3 text-foreground text-sm"
            value={config.telegramChannel}
            onChangeText={(v) => setConfig({ ...config, telegramChannel: v })}
          />
        </View>
        <View>
          <Text className="text-sm text-muted-foreground mb-1.5">Telegram Group</Text>
          <TextInput
            className="bg-background border border-border rounded-lg px-4 py-3 text-foreground text-sm"
            value={config.telegramGroup}
            onChangeText={(v) => setConfig({ ...config, telegramGroup: v })}
          />
        </View>
      </View>
    </ScrollView>
